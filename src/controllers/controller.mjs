import { getConnection } from "../database/database.cjs";
...
const getLastTotal = async (req, res) => {
  try {
    let sql = `SELECT id_total AS "id", total, usuarioid, fecha FROM totales WHERE usuarioid = ? ORDER by fecha DESC LIMIT 1`;
    const connection = await getConnection();
